...
def home_menu():
    ...
    print(f"{Fore.LIGHTYELLOW_EX}[4] {Fore.WHITE}Customize Password {Fore.RED}(Pro)\n")
...
## NEW UPDATE 2.0
    ...
    for num in range(1000001): 
        ...
        passwords.append(f"{digit}{first_name_ca}")
        passwords.append(f"{first_name_lo}{digit}")
        passwords.append(f"{digit}{first_name_lo}")
        passwords.append(f"{first_name_ti}{digit}")
        passwords.append(f"{digit}{first_name_ti}")
#Symbol_firstname
...
def customize_pro():
    first_nm = input(f"{Fore.LIGHTCYAN_EX}Enter First Name >> {Fore.RED}").title()
    middle_nm = input(f"{Fore.CYAN}Enter Middle Name {Fore.YELLOW}(if none then skip) >> {Fore.RED}").title()
    last_nm = input(f"{Fore.CYAN}Enter Last Name {Fore.YELLOW}(if none then skip) >> {Fore.RED}").title()
    info_1 = input(f"{Fore.BLUE}Do you add any target extra information {Fore.YELLOW}[if non then skip] >> {Fore.RED}").title()
    info_2 = input(f"{Fore.BLUE}Do you add any target extra information {Fore.YELLOW}[if non then skip] >> {Fore.RED}").title()
    info_3 = input(f"{Fore.BLUE}Do you add any target extra information {Fore.YELLOW}[if non then skip] >> {Fore.RED}").title()
    banner_1()
    random_case = (random.choice(string.ascii_uppercase)+random.choice(string.ascii_uppercase))
    file_name = (f"PRO_Passwordlist_{first_nm}_{random_case}.txt")
    passwords = []
    first_1 = first_nm[:1]
    last_1 = last_nm[:1]
    first_2 = first_nm[:2]
...
# New v-3.0 Last Name
    ...
    if info_1:
        passwords.append(f"{info_1}{digit}")
        passwords.append(f"{info_1.upper()}{digit}")
        passwords.append(f"{info_1.lower()}{digit}")
    if info_2:
        passwords.append(f"{info_2}{digit}")
        passwords.append(f"{info_2.upper()}{digit}")
        passwords.append(f"{info_2.lower()}{digit}") 
    if info_3:
        passwords.append(f"{info_3}{digit}")
        passwords.append(f"{info_3.upper()}{digit}")
        passwords.append(f"{info_3.lower()}{digit}")               
    passwords.append("@"*8)
    passwords.append("@"*9)
    passwords.append("@"*10)
    passwords.append("@"*11)
    ...
    passwords.append("*"*10)
    passwords.append("*"*11)
    passwords.append(("@"*4)+("#"*4))
    passwords.append(("#"*4)+("@"*4))
    passwords.append(("@"*5)+("#"*4))
    passwords.append(("@"*4)+("#"*5))
    passwords.append(("#"*5)+("@"*4))
    passwords.append(("#"*4)+("@"*5))
...
def main():
    ...
    if home_input == "1":
        ...
        last_name = input(Fore.CYAN+f"Target Last Name {Fore.YELLOW}(leave blank if none) >>{Fore.GREEN} ").strip()
